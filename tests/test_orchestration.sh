...
select_installed_identity() { record select_installed_identity; }
...
defer_aur_packages_install_mode() { record defer_aur_packages_install_mode; }
log_ok() { :; }
...
CALLS=''; CHECK_MODE=1; INSTALL_MODE=0
run_workstation
...
run_workstation
assert_eq "$install_expected" "${CALLS%$'\n'}" 'install mode uses installed identity, skips upgrade, and defers interactive AUR work'
